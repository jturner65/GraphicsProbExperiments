...
class myClassRoster extends mySampleSet{
	...
	protected my1D_DistVis[] gradeBars;// rawGradeBar, transGradeBar;
	...
	public myClassRoster(my_procApplet _pa, BaseProbExpMgr _gradeExp, String _name, float[][] _barLocs) {
		...
		gradeBars = new my1D_DistVis[transTypes.length];
		int barLocIDX = 0;
		for(int i=0;i<gradeBars.length;++i) {
			if(i>0) {barLocIDX=1;}
			gradeBars[i]=new my1D_DistVis(this, new float[] {_barLocs[barLocIDX][0], _barLocs[barLocIDX][1], distBtwnClassBars}, transTypes[i],clsLineClr, "Visualization of "+transTypes[i]+" grades for class :"+name);			
		}
...
//			}//for every bar			
	...
	protected void drawRawToTransformedLine(String fromTransType, String toTransType, myClassRoster fromCls, myClassRoster toCls, my1D_DistVis stGradeBar, float dist, boolean check) {
